...
# (the images should already exist on the machine; it was likely the build machine)
...
docker exec -u 0 -it porta bash -c "supervisorctl stop porta-worker:* && supervisorctl stop porta-task-worker:*"
