...
MAIN_MACHINE=$(get_machine_ip "$CONF_FILES_PATH/$BACKUP_CONF_FILE" "main");

# back-up the db first
docker exec -it porta bash -c "php artisan main:run --only-db --disable-notifications"
...
DB_FILE="/var/www/manual_db_dumps/backup_machine_all_database_$(date +'%Y%m%d.%H%M%S.%z').sql.gz"
