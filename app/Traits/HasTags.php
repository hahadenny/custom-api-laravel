...
trait HasTags
{
    ...
    {
        $tags = static::convertToTagsOfAnyType($this->company, $tags);
    ...
    public function attachTags(array | ArrayAccess | Tag $tags, string $type = null): static
    {
        ...
        $tags = collect($className::findOrCreate($this->company, $tags, $type));
    ...
    {
        $tags = static::convertToTags($this->company, $tags, $type);
    ...
    public function syncTags(string | array | ArrayAccess $tags): static
    {
        ...
        $tags = collect($className::findOrCreate($this->company, $tags));
    ...
    protected static function convertToTags(Company $company, $values, $type = null)
    {
        if ($values instanceof Tag) {
            $values = [$values];
        }

        return collect($values)->map(function ($value) use ($type, $company) {
            ...
            return $className::findFromString($company, $value, $type);
        });
    }

    protected static function convertToTagsOfAnyType(Company $company, $values)
    {
        return collect($values)->map(function ($value, $company) {
            if ($value instanceof Tag) {
                return $value;
            }
            ...
            return $className::findFromStringOfAnyType($company, $value);
        })->flatten();
    }
